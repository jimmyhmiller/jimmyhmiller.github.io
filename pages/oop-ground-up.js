...
const OopGroundUp = () => <GlobalLayout>
  ...
  <p>
    Any concept when taken for granted can lead to misunderstanding. Concepts
    ...
    programming, to the point where people are often confused at how languages
    with no objects can exist at all. We are going to peel back those covers
    and implement our own objects from the ground up.
  </p>
  <Heading size="2" text="The Simplest Object" />
  <p>
    In order to begin, we must have a target in mind for our first object. A
    "hello world" of objects, from there we will move to more advanced
    features implementing them ourselves along the way.
  </p>
  <Javascript>
  {`
    const person = {
  ...
  <p>
    ...
    as nice of a syntax as the one above, but we can definitely emulate the
    behavior of the object above.
  </p>
  <Javascript>
  {`
  ...
  <p>
    ...
    just how similar our "object" is to real objects.{" "}
  </p>
  ...
  <p>
    ...
    there was just a single abstraction, message passing (This is called the
  ...
  <p>
    Our first object (I hope you can see that I am justified in remove those
    quotes) was fairly limited. In order to make a new person with a different
    name, we would have to go write a whole new function. But our original
    javascript object had the same problem, while simpler in syntax, we still
    how to write out the whole thing. Let's fix that.
  </p>
  <Javascript>
  {`
  ...
  <p>
    Simple enough change, we just made a function that takes some parameters
    and returns our object. In fact, we can do the same for our object.
  </p>
  <Javascript>
  {`
    function createPerson(firstName, lastName) {
  ...
  <p>
    Since our object is just a function, we create a function that returns a
    function. Even with this "factory" function, our object continues to work
    just as it did before. But some of you may think, that's not a "real"
    object, "real" objects have methods. So let's add a method.
  </p>
  <Heading size="3" text="Methods" />
  <Javascript>
  {`
    function createPerson(firstName, lastName) {
      return {
  ...
  <p>
    Alright, there we are, an object with a method, this won't be too hard to
    recreate using our function technique.
  </p>
  <Javascript>
  {`
    function createPerson(firstName, lastName) {
  ...
  <p>
    That was simple enough. A method is really just a function. So all we need
    to do is have our object return a function when you access a property.
    Then you can call that function. Again though, some people might be
    saying, this isn't a "real" object, "real" objects encapsulate state.{" "}
  </p>
  <Heading size="3" text="State" />
  <Javascript>
  {`
  ...
  <p>
    ...
    with that feature left out, I hope some of the magic has been removed from
    objects. Objects aren't special. We don't need magic to make them, we just
    need simple functions.
  </p>
</GlobalLayout>;

export default OopGroundUp;