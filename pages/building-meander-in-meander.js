...
const BuildingMeanderInMeander = () => <GlobalLayout>
  ...
  <p>
    ...
    a clear separation of functionality, the code itself isn't a mess by any
    ...
    matches we've written, we have never once encountered the "method code
  ...
  <p>
    ...
    compiler is just converting between data structures, we read
  ...
  <p>
    ...
    accomplish. To do that, we are going to build our own
  ...
  <p>
    Before we get started though, let's talk about our plan. First to
    ...
    left as an exercise to the reader. To accomplish all of this
  ...
  <p>
    ...
    begin to see a bit of what the internals of meander looks like. But
  ...
  <p>
    ...
    nice, unambiguous maps. These maps will always have a <Term>:tag</Term>{" "}
  ...
  <p>
    This parser is very straightforward. We are taking advantage of meander's{" "}
  ...
  <p>
    ...
    interpreter will do is given an input and a pattern and an environment, we
    ...
    interpreter piece by piece, I will begin by showing you the entire thing.
  </p>
  <Clojure>
  {`
  ...
  <p>
    If you've written an interpreter before this shouldn't be too surprising.
    First we handle logic variables by looking them up in the environment. We
    ...
    does work for the input we've given it. But think about what would
    ...
    every single element of our vector. So let's try a different approach.
  </p>
  <Clojure>
  {`
    (defn interpret [expr target env]
  ...
  <p>
    ...
    be fairly easy, we check the size of <Term>?sequence</Term> and
  ...
  <p>
    ...
    have seen before. The translation is basically mechanical. Here
  ...
  <p>
    At first glance, it might be quite hard to spot the difference.
    There are only a few differences here. First and most crucially
    is that we have changed from <Term> m/match</Term> to{" "}
    <Term>m/rewrite</Term>. So now instead of our right-hand side being code
    that will be immediately run, it is actually data that we are outputting,
    in this case, that data is code. Secondly, we have quoted some of our
    symbols. Because we will ultimately be outputting this code in a macro and
...
`}
  ...
  <p>
    ...
    compilation returns us an expression that returns an environment. We build
...
`}
  ...
  <p>
    ...
    Rather than directly updating our <Term>?env</Term> by embedding more and
...
`}
  ...
  <p>
    ...
    <Term>?x</Term> will always succeed. Why are we checking there as well? For the case we are looking at now, we know that our input is a vector
  ...
  <p>
    ...
    zeta we've found meander's structured approach to help us
    understand our code and give us a clear sense of what to do next.
...
export default BuildingMeanderInMeander;
