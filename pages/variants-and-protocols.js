...
const VariantsAndProtocols = () => <GlobalLayout>
  ...
  <p>
    Above is our definition of the Maybe variant and its associated function,
    map. Just as map for a list applies a function to every element in the
  ...
  <p>
    Here is actually the map you are probably most familiar with, mapping over
    a list. And yet I'm sure this definition is new for many of you. List here
    is our own custom list instead of the Array from javascript. This list is
    a linked list, and sticking with the naming convention used for a long
    time, each link is built up using a constructor called <Term>Cons</Term>.
    In order to map over our list, we apply it to the first element and
  ...
  <p>
    Here we have a representation of a Tree. Mapping over a tree acts almost
  ...
  <p>
    This is the Identity variant. It has a completely trivial map function. We
    take out the x and apply f to it and then wrap it back up. This may seem
    pointless (there are uses), but it does show yet another use of map.
  </p>
  ...
  <p>
    ...
    Maybe, Either, or Identity, as long as we have an implementation of map.
  ...
  <p>
    Here is our first protocol, the Mapper protocol. Mapper is simple, in
    order to implement the Mapper protocol, you need to define map. One thing
    to note, however, is that our definition in the protocol does differ from
    our definitions before by one small detail, the arguments are flipped.
    Protocols require the type they are going to dispatch on to be the first
    argument. That is why we define a simple auxiliary function that flips them
    back around.
  </p>
  <p>
    Now that we've made our protocol and defined an implementation for Maybe,
    we can use it on any maybe values.
  </p>
  <Javascript>
  {`
  ...
  <p>
    What is special about this version of map is that as long as we define a
    Mapper implementation for a variant, we can pass a value of that variant
    to the map function and it will work.
  </p>
  <Javascript>
  {`
  ...
  <p>
    ...
    as to use old functions in new ways.
  </p>
  <Heading size="1" text="Conclusion" />
  <p>
    We have only seen a tiny glimpse into what protocols can do for us. The
    real power behind protocols comes when we group multiple fns together in a
    protocol and then build new functions that depend on that protocol. That
    may sound a bit abstract, but in our next post, we will dive in and
    implement a lodash like library that works on both built-in javascript
    data-structures and ImmutableJs data-structures all powered by protocols.
  </p>
</GlobalLayout>;

export default VariantsAndProtocols;