...
export async function getReadingsData() {
  ...
  // Whitelist-based filtering: anything not in includeCategories is hidden.
  // Track skipped categories so we can log them once at build time.
  const allowlist = new Set(config.includeCategories || []);
  const seenCategories = new Set();
  const skippedCategories = new Set();
  const categoryMap = {};

  for (const pdf of s3PDFs) {
    seenCategories.add(pdf.category);

    if (!allowlist.has(pdf.category)) {
      skippedCategories.add(pdf.category);
      continue;
    }
  ...
  // Surface category drift: new categories in S3 not yet whitelisted, and
  // whitelisted categories with no files in S3 (likely renamed or removed).
  if (skippedCategories.size > 0) {
    const list = [...skippedCategories].sort().map((c) => `  - ${c}`).join('\n');
    console.warn(`[readings] Skipped ${skippedCategories.size} S3 categories not in includeCategories:\n${list}\n  Add to readings-config.js if you want them shown.`);
  }
  const missing = [...allowlist].filter((c) => !seenCategories.has(c));
  if (missing.length > 0) {
    console.warn(`[readings] Whitelisted categories with no S3 files: ${missing.join(', ')}`);
  }
